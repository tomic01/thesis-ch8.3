...
DISTANCE_FAR_FROM_LEADER_THRESHOLD = 3.0    
DISTANCE_CLOSE_TO_LEADER_THRESHOLD = 1.5
...
class StateWaitForTeammates(smach.State):
  ...
  def execute(self, userdata):
    ...
    preempted = False
    while not rospy.is_shutdown() and all(self.follower_ready) is False and  preempted is False:
...
class StateCheckTeammates(smach.State):
  ...
  def execute(self, userdata):
    ...
    # loop until all followers ready or preempted
    preempted = False
    while not rospy.is_shutdown() and any(self.follower_not_ready) is False and preempted is False:
      if self.preempt_requested():
        self.service_preempt()
        preempted = True
...
class StateGoToLeader(smach.State):
  ...
  def execute(self, userdata):
    ...
    while self.leader_found is False:
      rospy.sleep(0.5)
...
if __name__ == '__main__':
  ...
  # Waypoints at IST with the living room (Alicja)
  ...
  # Waypoints at IST with corridor (Alicja)
  # waypoints = [(-6.3, 8.3, 0.48),
  #              (2.75,12.85,0.3),
  #              (11.75, 9.2, -1.09),
  #              (7.15, -2.6, -2.55)]  

  # Waypoints at IST with corridor and living room (Alicja revisited), A B C B A D
  waypoints = [(-3.4, 9.3, 0.48),
               (4.45, 13.2, 0.48),
               (6.05, -0.8, 0.55),
               (4.45, 13.2, 0.48),
               (-3.4, 9.3, 0.48),
