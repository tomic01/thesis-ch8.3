...
from geometry_msgs.msg import Pose2D, PoseWithCovariance, PoseWithCovarianceStamped, TwistStamped
...
class Start(MonarchState):
	def __init__(self):
		MonarchState.__init__(self, state_outcomes=['succeeded','preempted'],input_keys=['action_goal','distance','cannot_read'], output_keys=['distance','cannot_read'])
  	...
  	def execute(self, userdata):
		...
		if len(userdata.action_goal.goal.array)>1:
			global vel
			global vel_aux
			vel = float(userdata.action_goal.goal.array[1].value)
			vel_aux=vel
...
class MoveToEnd(MonarchState):
    def __init__(self):
        MonarchState.__init__(self, state_outcomes=['succeeded','preempted','aborted'], input_keys=[''],output_keys=[''])
	rospy.loginfo('Init state MOVETOEND')
	navMoveToPersonAction = NavByVelRefAction(self, 'move_back')
	self.add_action(navMoveToPersonAction)

# define state SENDMOTION
class SendMotion(MonarchState):
    def __init__(self):
        MonarchState.__init__(self, state_outcomes=['succeeded','person_too_far','preempted'], input_keys=['robot_position','person_position'],output_keys=['robot_position','person_position'])
	rospy.loginfo('Init state SENDMOTION')
	self.pub = rospy.Publisher('move_back', TwistStamped, queue_size=10)


    def execute(self, userdata):
        rospy.loginfo('Executing state SENDMOTION')

  	if self.preempt_requested():
        	self.service_preempt()
               	return 'preempted'

	topic_value=TwistStamped()
	topic_value.header.frame_id='base_link'


	angle=atan2((userdata.person_position.y-userdata.robot_position.y),(userdata.person_position.x-userdata.robot_position.x))
	while angle<0.0:
		angle=angle+2*pi
	while angle>2*pi:
		angle=angle-2*pi
	angle_robot=userdata.robot_position.theta
	while angle_robot<0.0:
		angle_robot=angle_robot+2*pi
	while angle_robot>2*pi:
		angle_robot=angle_robot-2*pi
	dif_angles=angle-angle_robot
	if abs(dif_angles)<pi/8 or abs(dif_angles)>2*pi-pi/8:
		topic_value.twist.angular.x=0.0
		topic_value.twist.angular.y=0.0
		topic_value.twist.angular.z=0.0	
	elif (dif_angles>0 and dif_angles<pi) or dif_angles<-pi:
		topic_value.twist.angular.x=0.0
		topic_value.twist.angular.y=0.0
		topic_value.twist.angular.z=0.4	
	else:
		topic_value.twist.angular.x=0.0
		topic_value.twist.angular.y=0.0
		topic_value.twist.angular.z=-0.4	
	...
	if sqrt((userdata.person_position.x-userdata.robot_position.x)*(userdata.person_position.x-userdata.robot_position.x)+(userdata.person_position.y-userdata.robot_position.y)*(userdata.person_position.y-userdata.robot_position.y))>3.0:
		topic_value.twist.linear.x=0.0
		topic_value.twist.linear.y=0.0
		topic_value.twist.linear.z=0.0
		self.pub.publish(topic_value)
        	return 'person_too_far'
	else:
		topic_value.twist.linear.x=-vel_aux #-vel_aux*cos(userdata.robot_position.theta)
		topic_value.twist.linear.y=0.0 #-vel_aux*sin(userdata.robot_position.theta)
		topic_value.twist.linear.z=0.0
		self.pub.publish(topic_value)
        	return 'succeeded'
...
if __name__ == '__main__':
    ...
    rospy.init_node('behavior_catchandtouchback')
	...
	sm2.userdata=sm.userdata
   	with sm2:
    	...
    	sm3 = smach.StateMachine(outcomes = ['preempted','person_too_far'])
	sm3.userdata=sm.userdata
   	with sm3:
      		sm3.add('LOOKHUMAN', LookHuman(),
                        transitions = { 'preempted':'preempted','succeeded':'LOOKROBOT'})
		sm3.add('LOOKROBOT', LookRobot(),
			transitions = {'preempted':'preempted','person_too_far':'person_too_far','person_following':'LOOKHUMAN'})


        def termination_cb2(outcome_map):
            if outcome_map['MONITOR_POS'] == 'person_near' or outcome_map['MONITOR_POS'] == 'person_too_far' or outcome_map['WAIT'] == 'wait_finished':
                return True
            else:
                return False

	sm_con2 = smach.Concurrence(outcomes=['person_too_far','person_lost','monitor_default','preempted'],
                                      default_outcome='monitor_default',
				      child_termination_cb = termination_cb2,
                                      outcome_map={'person_too_far':
	...
	sm_con2.userdata=sm.userdata
        with sm_con2:
            smach.Concurrence.add('MONITOR_POS', sm3)
	    smach.Concurrence.add('WAIT',Wait())


    	sm7 = smach.StateMachine(outcomes = ['preempted'])
	sm7.userdata=sm.userdata
   	with sm7:
      		sm7.add('MONITOR_SUB', sm_con2,
                        transitions = { 'person_too_far':'MONITOR_SUB','person_lost':'MONITOR_SUB','monitor_default':'MONITOR_SUB','preempted':'preempted'})

    	sm8 = smach.StateMachine(outcomes = ['person_lost','preempted'])
	sm8.userdata=sm.userdata
   	with sm8:
      		sm8.add('SENDMOTION_SUB', SendMotion(),
                        transitions = { 'succeeded':'SENDMOTION_SUB','person_too_far':'person_lost','preempted':'preempted'})
	...
	sm6.userdata=sm.userdata
   	with sm6:
        ...
        def termination_cb1(outcome_map):
            if outcome_map['SENDMOTION'] == 'person_lost' or outcome_map['MOVETOEND'] == 'aborted' or outcome_map['ACCELERATE'] == 'catched':
                return True
            else:
                return False

        sm_con1 = smach.Concurrence(outcomes=['lead_default','lead_catched','look_for_person','lead_aborted','lead_preempted'],
                                      default_outcome='lead_default',
				      child_termination_cb = termination_cb1,
                                      outcome_map={'lead_catched':
                                          {'ACCELERATE':'catched'},
                                              'look_for_person':
                                          {'MOVETOEND':'preempted',
				            'SENDMOTION':'person_lost',
						'ACCELERATE':'preempted'},
                                               'lead_aborted':
                                          {'MOVETOEND':'aborted'},
                                               'lead_preempted':
                                          {'MOVETOEND':'preempted',
                                            'SENDMOTION':'preempted',
						'CHECK':'preempted',
					   'ACCELERATE':'preempted',
					    'MONITOR':'preempted'}})
	...
	sm5.userdata=sm.userdata
   	with sm5:
        ...
        def termination_cb3(outcome_map):
            if outcome_map['MONITOR_POS2'] == 'person_near' or outcome_map['WAITMORE'] == 'wait_finished':
                return True
            else:
                return False

	sm_con3 = smach.Concurrence(outcomes=['person_detected_pos','person_lost','monitor_default','monitor_preempted'],
                                      default_outcome='monitor_default',
	...
	sm_con3.userdata=sm.userdata
        ...
        sm.add('GAME',sm_con1, transitions={'lead_default':'GAME','lead_catched':'CATCHED','look_for_person':'ENCOURAGE','lead_aborted':'ABORT','lead_preempted':'preempted'})

        sm.add('ENCOURAGE', Encourage(), transitions={'succeeded':'WAITING_PERSON'})

        sm.add('WAITING_PERSON',sm_con3, transitions={'person_detected_pos':'GAME','person_lost':'FINISH','monitor_preempted':'preempted','monitor_default':'WAITING_PERSON'})
