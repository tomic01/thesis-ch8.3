...
from monarch_behaviors.msg import PatrollingApproachInteractCameraAction
...
class FindRfidTag(MonarchState):
	def __init__(self):
		MonarchState.__init__(self, state_outcomes = ['succeeded','preempted','unsucceeded'], input_keys=[''],output_keys=[''])
		rospy.loginfo("FindRfidTag")
		topicName = 'rfid_tag'
		rfidCondition = """
if data.tag_id == 5275 or data.tag_id == 5267 or data.tag_id == 6892:
	value = 'succeeded'
else:
	value = 'unsucceeded'
"""
		condition_outcomes = ['succeeded','preempted','unsucceeded']
		tc = TopicCondition(self, topicName, RfidReading, rfidCondition)
		self.add_change_condition(tc, condition_outcomes)

class SimpleActivateCaState(smach.State):
	def __init__(self):
		smach.State.__init__(self, outcomes = ['succeeded'])
		self.publisher = rospy.Publisher('ca_activations', KeyValuePairArray)
...
"""
		...
		condition_outcomes = ['succeeded','preempted','unsucceeded']
		tc = TopicCondition(self, topicName, Bool, topicCondition)
		self.add_change_condition(tc, condition_outcomes)
		runGreetingCaState = RunCaAction(self, 'ca15')
...
class ApproachStateStart(smach.State):
	...
	def execute(self, userdata):
		...
		userdata.start_pose_out = pose2pose_stamped(0.0,0.0,0.0)
...
if __name__ == '__main__':
	rospy.init_node('behavior_patrollingapproachinteractcamera')
	...
	sm_approach = smach.StateMachine(outcomes = ['user_reached','preempted','aborted'],
	...
	with sm_approach:
		sm_approach.add('MOVE',
						smach_ros.SimpleActionState('move_base',
													...
													transitions = {'succeeded':'user_reached'},
													remapping = {'target_pose':'pose_stamped'})


	#	State Machine Start(Find person)
	sm_start_approach = smach.StateMachine(outcomes = ['succeeded','preempted'],
											output_keys = ['pose_stamped'])
	with sm_start_approach:
		sm_start_approach.add('START',
							ApproachStateStart(),
							transitions = {'user_position_set':'succeeded',
										   'start_preempted':'preempted'},
							remapping = {'start_pose_out':'pose_stamped'})

	#	State Machine RUN CA
	sm_ca = smach.StateMachine(outcomes = ['succeeded','preempted','aborted','unsucceeded'])	
	with sm_ca:
		sm_ca.add('ActivateCaState',
					SimpleActivateCaState(),
					transitions = {'succeeded':'RunCaState'})
		sm_ca.add('RunCaState',
					RunCaState())
	...
	with sm_getrfid:
		sm_getrfid.add('GETRFIDSTATE',
					   FindRfidTag(),
					   transitions = {'unsucceeded':'GETRFIDSTATE'})
	...
	def termination_cm_1_cb(outcome_map):
		if outcome_map['STARTAPPROACHSM'] == 'succeeded':
			return True
	...
	with cm_1:
		...
		cm_1.add('STARTAPPROACHSM',sm_start_approach)
	...
	def termination_cm_2_cb(outcome_map):
		if outcome_map['APPROACH'] == 'user_reached' or outcome_map['APPROACH'] == 'preempted' or outcome_map['APPROACH'] == 'aborted':
			return True
	...
	def outcome_cm_2_cb(outcome_map):
		if outcome_map['APPROACH'] == 'user_reached':
			return 'user_reached'
		if outcome_map['GETRFIDSM'] == 'succeeded' or outcome_map['GETRFIDTIMERSM'] == 'succeeded':
			return 'succeeded'
	...
	cm_2 = smach.Concurrence(outcomes = ['user_reached','succeeded','preempted','aborted'],
							 default_outcome = 'user_reached',
							 input_keys = ['pose_stamped'],
	...
	with mm:
		...
		mm.add('APPROACHRFIDTIMER',
				...
				transitions = {'succeeded':'RUNCA',
							   'user_reached':'PATROLANDTIMER'})
				
		mm.add('RUNCA',
				sm_ca,
				transitions = {'succeeded':'PATROLANDTIMER',
							   'unsucceeded':'PATROLANDTIMER'})
	...
	asw = smach_ros.ActionServerWrapper(action_name,
										PatrollingApproachInteractCameraAction,
