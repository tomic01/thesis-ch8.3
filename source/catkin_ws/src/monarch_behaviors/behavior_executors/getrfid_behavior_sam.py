...
from monarch_behaviors.msg import GetRfidAction, GetRfidResult, GetRfidGoal
...
ident=0
tracker_slot="People_Localization_Tracker"

# define state START
class Start(MonarchState):
	def __init__(self):
		MonarchState.__init__(self, state_outcomes=['succeeded','preempted'], input_keys=['init_person_position','distance','cannot_read','action_goal'], output_keys=['init_person_position','distance','cannot_read'])
  	...
  	def execute(self, userdata):
		...
		if len(userdata.action_goal.goal.array)>0:
			if userdata.action_goal.goal.array[0].key=='Tracker_slot':
				tracker_slot=userdata.action_goal.goal.array[0].value
			else:
				ident = int(userdata.action_goal.goal.array[0].value)

 		if len(userdata.action_goal.goal.array)>1:
			if userdata.action_goal.goal.array[1].key=='Tracker_slot':
				tracker_slot=userdata.action_goal.goal.array[1].value
			else:
				ident = int(userdata.action_goal.goal.array[1].value)   		

    		sam_reader= SAMReader(tracker_slot,self.people_localization_tracker_cb)
...
class Finish(MonarchState):
	...
	if len(userdata.action_result.result.array)>0:
		userdata.action_result.result.array[0].key = 'got'
...
if __name__ == '__main__':
    ...
    with sm:
        ...
        sm.add('SET', Set(), 
                               transitions={'final_position_set':'APPROACH', 'too_near':'ABORT', 'preempted':'preempted'})
	...
	sm_con1.userdata=sm.userdata
        ...
        sm.add('APPROACH',sm_con1,
                                transitions={'approach_default':'APPROACH','approach_done':'GET','position_wrong':'START','rfid_read':'FINISH', 'approach_aborted':'ABORT','approach_preempted':'preempted'})
	...
	sm_con2.userdata=sm.userdata
        ...
        sm.add('GET',sm_con2,
                                transitions={'get_default':'GET','get_succeeded':'FINISH','get_failed':'SET', 'get_aborted':'ABORT','get_preempted':'preempted'})
